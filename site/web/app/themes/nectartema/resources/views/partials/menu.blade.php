<div x-data="{ mobileOpen: false }" class="relative" x-cloak>
    ...
    <ul class="hidden lg:flex justify-between text-xl lg:text-lg font-heading">
        @foreach ($primary_navigation as $item)
            <li class="hover:text-primary hover:underline transition-colors">
                <a href="{{ $item->url }}"
                  class="hover:text-secondary {{ $item->active ? 'text-primary font-semibold' : 'text-gray-800' }}"
    ...
    <button @click.stop="mobileOpen = !mobileOpen" class="lg:hidden p-2 relative w-10 h-10 z-50 left-12 top-4"
        ...
        <div class="container p-4">
            @foreach ($primary_navigation as $item)
                <a href="{{ $item->url }}" @click="mobileOpen = false"
                    class="block py-3 text-xl border-b border-gray-100 {{ $item->active ? 'text-primary' : 'text-gray-800' }}"
                    @if ($item->active || $item->activeAncestor) aria-current="{{ $item->active ? 'page' : 'true' }}" @endif>
                    {{ $item->label }}
                </a>
            @endforeach
